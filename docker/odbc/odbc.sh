...
ACCEPT_EULA=Y microdnf install -y tar gzip openssl libaio libnsl unixODBC unixODBC-devel msodbcsql17 msodbcsql18 mariadb-connector-odbc postgresql-odbc && \
...
echo "LD_LIBRARY_PATH=/usr/lib/oracle/11.2/client64/lib/:/usr/lib64/:/usr/local/bin:/usr/local/sbin:/usr/sbin:/usr/bin/" >> /etc/profile && \
